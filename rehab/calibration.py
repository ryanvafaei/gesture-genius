...
and then records the step's values for CALIBRATION_HOLD_S. The median over
the hold is stored, which ignores single bad frames. Frames with a quality
problem (hand lost, wrong hand, too far) are skipped and the hold timer
pauses, so she never has to start over.
...
class CalibrationRoutine:
    ...
    def update(self, features, now, quality_ok=True):
        ...
        if self._stage == "settle":
            if now - self._stage_t >= self.settle_s:
                self._stage = "hold"
            return []
        ...
        if self._held >= self.hold_s and len(self._samples) >= self.min_samples:
            ...
            return [Say("And relax.", "count")] + self._next(now)
        return []
